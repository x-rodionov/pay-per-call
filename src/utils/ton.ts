import TonWeb, { type AddressType, type WalletContractMethods } from 'tonweb';
import tonMnemonic, { type KeyPair } from 'tonweb-mnemonic';
...
export async function getKeyPairAndWallet(mnemonics: string[], password?: string) {
	...
	if ((await tonMnemonic.isPasswordNeeded(mnemonics)) && password == undefined) {
		throw new Error('This mnemonic requires a password but none was provided');
	}
	const keyPair = await tonMnemonic.mnemonicToKeyPair(mnemonics, password);
